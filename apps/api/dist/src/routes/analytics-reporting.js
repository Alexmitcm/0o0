...
const analyticsReporting = new hono_1.Hono();
...
const generateReportSchema = zod_1.z.object({
    ...
    format: zod_1.z.enum(["json", "csv", "excel"]).default("json"),
...
const userAnalyticsSchema = zod_1.z.object({
    endDate: zod_1.z.string().datetime().optional(),
...
analyticsReporting.get("/dashboard-stats", authMiddleware_1.default, async (c) => {
    try {
        const [totalUsers, activeUsers, premiumUsers, bannedUsers, totalCoins, totalTransactions, totalTournaments, totalWithdrawals, recentUsers, topUsers] = await Promise.all([
            prisma.user.count(),
            prisma.user.count({
                where: {
                    lastActiveAt: {
...
analyticsReporting.get("/user-analytics", authMiddleware_1.default, async (c) => {
    try {
...
analyticsReporting.get("/coin-analytics", authMiddleware_1.default, async (c) => {
    try {
        ...
        // Calculate totals
        const totalCoins = coinBalances.reduce((sum, balance) => sum + balance.totalCoins, 0);
        const totalExperienceCoins = coinBalances.reduce((sum, balance) => sum + balance.experienceCoins, 0);
        const totalAchievementCoins = coinBalances.reduce((sum, balance) => sum + balance.achievementCoins, 0);
        const totalSocialCoins = coinBalances.reduce((sum, balance) => sum + balance.socialCoins, 0);
        const totalPremiumCoins = coinBalances.reduce((sum, balance) => sum + balance.premiumCoins, 0);
...
analyticsReporting.get("/tournament-analytics", authMiddleware_1.default, async (c) => {
    try {
        ...
        const totalPrizeDistributed = prizeDistribution.reduce((sum, p) => sum + Number(p.prizeAmount || 0), 0);
        const totalCoinsBurned = prizeDistribution.reduce((sum, p) => sum + Number(p.coinsBurned), 0);
        return c.json({
            data: {
                participants: participantStats,
                summary: {
                    averageParticipantsPerTournament: participants.length / tournaments.length || 0,
                    totalCoinsBurned,
...
analyticsReporting.post("/generate-report", authMiddleware_1.default, async (c) => {
    try {
        const body = await c.req.json();
        const { reportType, startDate, endDate, format, filters } = generateReportSchema.parse(body);
        let data = [];
        let headers = [];
        ...
        if (format === "csv" || format === "excel") {
            ...
            c.header("Content-Disposition", `attachment; filename="${reportType}_report.${format}"`);
...
analyticsReporting.get("/export-users-csv", authMiddleware_1.default, async (c) => {
    try {
...
analyticsReporting.get("/export-transactions-csv", authMiddleware_1.default, async (c) => {
    try {
...
analyticsReporting.get("/real-time-stats", authMiddleware_1.default, async (c) => {
    try {
        const now = new Date();
        const oneHourAgo = new Date(now.getTime() - 60 * 60 * 1000);
        const oneDayAgo = new Date(now.getTime() - 24 * 60 * 60 * 1000);
        const [activeUsersLastHour, activeUsersLastDay, transactionsLastHour, transactionsLastDay, newUsersLastHour, newUsersLastDay] = await Promise.all([
            prisma.user.count({
                where: {
...
exports.default = analyticsReporting;
