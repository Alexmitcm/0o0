...
const visNetwork = new hono_1.Hono();
// Validation schemas
const networkFilterSchema = zod_1.z.object({
    banned: zod_1.z.boolean().optional(),
    limit: zod_1.z.number().int().positive().max(1000).default(500),
    maxEq: zod_1.z.number().int().min(0).optional(),
    minEq: zod_1.z.number().int().min(0).optional(),
    status: zod_1.z.enum(["Premium", "Standard"]).optional()
});
// GET /network - Get network data for Vis.js
visNetwork.get("/network", authMiddleware_1.default, async (c) => {
    try {
        const query = c.req.query();
        const { minEq, maxEq, status, banned, limit } = networkFilterSchema.parse(query);
        // Build where clause
        const where = {};
        if (minEq !== undefined)
            where.totalEq = { gte: minEq };
        if (maxEq !== undefined)
            where.totalEq = { ...where.totalEq, lte: maxEq };
        if (status !== undefined)
            where.status = status;
        if (banned !== undefined)
            where.banned = banned;
...
visNetwork.get("/network/user/:walletAddress", async (c) => {
    try {
        ...
        // Get referral tree
        const getReferralTree = async (userWallet, currentDepth) => {
            if (currentDepth <= 0)
                return [];
            ...
            for (const referral of referrals) {
                const childTree = await getReferralTree(referral.walletAddress, currentDepth - 1);
                children.push({
                    ...referral,
                    children: childTree
                });
        ...
        const nodes = [user, ...referrals.flatMap((r) => [r, ...r.children])].map((user) => ({
            color: {
                background: user.banned
                    ? "#ff6b6b"
                    : user.status === "Premium"
...
visNetwork.get("/network/stats", authMiddleware_1.default, async (c) => {
    try {
        const [totalUsers, totalReferrals, premiumUsers, bannedUsers, avgReferralsPerUser, topReferrers, networkDensity] = await Promise.all([
            prisma.user.count(),
        ...
        return c.json({
            stats: {
                avgReferralsPerUser: Math.round(((avgReferralsPerUser._avg.leftNode || 0) +
...
visNetwork.get("/network/export", authMiddleware_1.default, async (c) => {
    try {
        ...
        if (format === "csv") {
            ...
            const csvRows = data.network.nodes.map((node) => [
                node.id,
                node.label,
                node.group,
                node.size,
