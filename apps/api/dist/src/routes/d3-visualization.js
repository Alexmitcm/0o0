...
const d3Visualization = new hono_1.Hono();
// Validation schemas
...
d3Visualization.get("/tree", authMiddleware_1.default, async (c) => {
    try {
        ...
        const calculateStats = (node) => {
            ...
            node.stats = {
                activeChildren,
                conversionRate: totalChildren > 0 ? (activeChildren / totalChildren) * 100 : 0,
                totalChildren,
                totalCoins,
                totalEq
            };
...
d3Visualization.get("/network", authMiddleware_1.default, async (c) => {
    try {
...
d3Visualization.get("/stats", authMiddleware_1.default, async (c) => {
    try {
        const [totalUsers, premiumUsers, bannedUsers, totalReferrals, avgReferralsPerUser, topReferrers] = await Promise.all([
            prisma.user.count(),
...
d3Visualization.get("/export/:walletAddress", async (c) => {
    try {
        ...
        const response = await d3Visualization.fetch(`/tree/${walletAddress}?depth=${depth}`);
...
d3Visualization.get("/heatmap", authMiddleware_1.default, async (c) => {
    try {
...
exports.default = d3Visualization;
