...
const coinSystem = new hono_1.Hono();
// Validation schemas
const coinUpdateSchema = zod_1.z.object({
    walletAddress: zod_1.z.string().regex(/^0x[a-fA-F0-9]{40}$/),
    amount: zod_1.z.number().int().positive(),
    sourceType: zod_1.z.enum(['Registration', 'Referral', 'Quest', 'Activity', 'Social', 'GamePlay', 'Tournament', 'Admin', 'Bonus', 'Achievement', 'DailyLogin', 'WeeklyChallenge', 'LootBox', 'MonthlyReward']).default('Activity'),
    sourceId: zod_1.z.string().optional(),
    sourceMetadata: zod_1.z.record(zod_1.z.any()).optional(),
});
const coinTransferSchema = zod_1.z.object({
    fromWallet: zod_1.z.string().regex(/^0x[a-fA-F0-9]{40}$/),
    toWallet: zod_1.z.string().regex(/^0x[a-fA-F0-9]{40}$/),
    amount: zod_1.z.number().int().positive(),
    coinType: zod_1.z.enum(['Experience', 'Achievement', 'Social', 'Premium']).default('Experience'),
    description: zod_1.z.string().optional(),
});
const userInsertCoinSchema = zod_1.z.object({
    walletAddress: zod_1.z.string().regex(/^0x[a-fA-F0-9]{40}$/),
    amountOfCoin: zod_1.z.number().int().positive(),
    tournamentId: zod_1.z.string(),
});
// Helper function to check captcha restrictions
async function checkCaptchaRestrictions(walletAddress) {
...
async function calculateStaminaLevel(walletAddress) {
    const user = await prisma.user.findUnique({
        where: { walletAddress },
...
async function checkDailyLimits(walletAddress) {
    const user = await prisma.user.findUnique({
        where: { walletAddress },
...
coinSystem.post('/coin-transfer', async (c) => {
    try {
        ...
        if (!fromUser || !toUser) {
            throw new ApiError_1.ApiError('One or both users not found', 404);
        }
        ...
        if (!fromBalance || fromBalance.totalCoins < amount) {
            throw new ApiError_1.ApiError('Insufficient coins for transfer', 400);
        }
...
coinSystem.post('/reset-daily-points', authMiddleware_1.default, async (c) => {
    try {
