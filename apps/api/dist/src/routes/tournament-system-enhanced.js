...
const tournamentSystem = new hono_1.Hono();
// Validation schemas
const createTournamentSchema = zod_1.z.object({
    gameName: zod_1.z.string().min(1),
    startDate: zod_1.z.string().datetime(),
    endDate: zod_1.z.string().datetime(),
    tagForSeo: zod_1.z.string().optional(),
    minimumCoin: zod_1.z.number().positive().optional(),
    minimumRefer: zod_1.z.number().int().positive().optional(),
    maximumRefer: zod_1.z.number().int().positive().optional(),
    storageCapacity: zod_1.z.number().int().positive().optional(),
    tournamentPrize: zod_1.z.number().positive().optional(),
    type: zod_1.z.enum(['Balanced', 'Unbalanced']).default('Balanced'),
    prizePool: zod_1.z.number().positive().optional(),
    minCoins: zod_1.z.number().positive().optional(),
    equilibriumMin: zod_1.z.number().int().positive().optional(),
    equilibriumMax: zod_1.z.number().int().positive().optional(),
...
const joinTournamentSchema = zod_1.z.object({
    ...
    coinsBurned: zod_1.z.number().positive().optional(),
});
const disableTournamentSchema = zod_1.z.object({
    tournamentId: zod_1.z.string(),
});
...
async function checkUserEligibility(walletAddress, tournament) {
    try {
...
tournamentSystem.post('/create', authMiddleware_1.default, async (c) => {
    try {
...
tournamentSystem.get('/:id', async (c) => {
    try {
        ...
        if (!tournament) {
            throw new ApiError_1.ApiError('Tournament not found', 404);
        }
        return c.json({
            success: true,
            tournament
...
tournamentSystem.post('/join', async (c) => {
    try {
        ...
        if (tournament.isDisabled) {
            throw new ApiError_1.ApiError('Tournament is disabled', 400);
        }
        // Check if tournament is still open
        const now = new Date();
        if (now < tournament.startDate) {
            throw new ApiError_1.ApiError('Tournament has not started yet', 400);
        }
        if (now > tournament.endDate) {
            throw new ApiError_1.ApiError('Tournament has ended', 400);
        }
        // Check user eligibility
        const eligibility = await checkUserEligibility(walletAddress, tournament);
        if (!eligibility.eligible) {
            throw new ApiError_1.ApiError(eligibility.reason || 'Not eligible for tournament', 400);
        }
        // Check if user already joined
        ...
        if (existingParticipation) {
            throw new ApiError_1.ApiError('Already joined tournament', 400);
        }
        // Join tournament
...
tournamentSystem.post('/disable', authMiddleware_1.default, async (c) => {
    try {
        const body = await c.req.json();
...
tournamentSystem.post('/settle', authMiddleware_1.default, async (c) => {
    try {
        const body = await c.req.json();
        const { tournamentId, settlementTxHash } = zod_1.z.object({
            tournamentId: zod_1.z.string(),
            settlementTxHash: zod_1.z.string().optional()
        }).parse(body);
        const tournament = await prisma.tournament.findUnique({
            where: { id: tournamentId },
            include: {
        ...
        if (tournament.status === 'Settled') {
            throw new ApiError_1.ApiError('Tournament already settled', 400);
        }
        ...
        if (participants.length === 0) {
            throw new ApiError_1.ApiError('No participants with coins to settle', 400);
        }
        ...
        await prisma.$transaction(updatedParticipants.map(participant => prisma.tournamentParticipant.update({
...
exports.default = tournamentSystem;
