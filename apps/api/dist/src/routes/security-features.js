...
const securityFeatures = new hono_1.Hono();
// Validation schemas
const captchaRequestSchema = zod_1.z.object({
    walletAddress: zod_1.z.string().regex(/^0x[a-fA-F0-9]{40}$/),
    token: zod_1.z.string(),
});
const captchaSolveSchema = zod_1.z.object({
    walletAddress: zod_1.z.string().regex(/^0x[a-fA-F0-9]{40}$/),
    captchaData: zod_1.z.string(),
    solution: zod_1.z.string(),
});
const banUserSchema = zod_1.z.object({
    walletAddress: zod_1.z.string().regex(/^0x[a-fA-F0-9]{40}$/),
    reason: zod_1.z.string(),
    duration: zod_1.z.number().int().positive().default(24), // hours
});
const unbanUserSchema = zod_1.z.object({
    walletAddress: zod_1.z.string().regex(/^0x[a-fA-F0-9]{40}$/),
});
const checkBanSchema = zod_1.z.object({
    walletAddress: zod_1.z.string().regex(/^0x[a-fA-F0-9]{40}$/),
});
...
async function checkBanConditions(walletAddress) {
...
securityFeatures.get('/captcha', async (c) => {
    try {
        ...
        if (!walletAddress || !token) {
            throw new ApiError_1.ApiError('Missing wallet address or token', 400);
        }
        // Validate user token
        const user = await prisma.user.findUnique({
            where: { walletAddress },
        ...
        const captchaData = (0, crypto_1.createHash)('sha256').update(`${question}-${answer}-${Date.now()}`).digest('hex');
...
securityFeatures.post('/captcha/solve', async (c) => {
    try {
        ...
        if (!captcha) {
            throw new ApiError_1.ApiError('Captcha challenge not found', 404);
        }
        ...
        if (timeSinceCreation > captchaTimeout) {
            throw new ApiError_1.ApiError('Captcha expired', 400);
        }
        ...
        if (isCorrect) {
            ...
            throw new ApiError_1.ApiError('Incorrect captcha solution', 400);
        }
    }
    catch (error) {
        return (0, handleApiError_1.default)(c, error);
...
securityFeatures.post('/ban-user', authMiddleware_1.default, async (c) => {
    try {
        const body = await c.req.json();
...
securityFeatures.post('/unban-user', authMiddleware_1.default, async (c) => {
    try {
        const body = await c.req.json();
...
securityFeatures.get('/security-stats', authMiddleware_1.default, async (c) => {
    try {
        const [totalBannedUsers, totalCheatAttempts, totalCaptchaChallenges, solvedCaptchaChallenges, recentBans] = await Promise.all([
...
securityFeatures.post('/reset-cheat-count', authMiddleware_1.default, async (c) => {
    try {
        const body = await c.req.json();
        const { walletAddress } = zod_1.z.object({ walletAddress: zod_1.z.string() }).parse(body);
        await prisma.user.update({
...
securityFeatures.post('/bulk-ban', authMiddleware_1.default, async (c) => {
    try {
        const body = await c.req.json();
        const { walletAddresses, reason, duration } = zod_1.z.object({
            walletAddresses: zod_1.z.array(zod_1.z.string()),
            reason: zod_1.z.string(),
            duration: zod_1.z.number().int().positive().default(24)
        }).parse(body);
        const results = [];
        const errors = [];
...
exports.default = securityFeatures;
