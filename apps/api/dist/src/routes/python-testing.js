...
const pythonTesting = new hono_1.Hono();
// Validation schemas
const testConfigSchema = zod_1.z.object({
    data: zod_1.z.record(zod_1.z.any()).optional(),
    delay: zod_1.z.number().min(0).max(10).default(0.1),
    endpoint: zod_1.z.string(),
    headers: zod_1.z.record(zod_1.z.string()).optional(),
    iterations: zod_1.z.number().int().positive().max(1000).default(100),
    method: zod_1.z.enum(["GET", "POST", "PUT", "DELETE"]).default("GET")
});
// POST /run-tests - Run automated API tests
pythonTesting.post("/run-tests", authMiddleware_1.default, async (c) => {
    try {
        const body = await c.req.json();
        const { endpoint, method, data, headers, iterations, delay } = testConfigSchema.parse(body);
        const results = {
            delay,
            endpoint,
            iterations,
...
pythonTesting.get("/test-endpoints", authMiddleware_1.default, async (c) => {
    try {
        const endpoints = [
            {
...
pythonTesting.get("/test-results", authMiddleware_1.default, async (c) => {
    try {
...
pythonTesting.post("/generate-test-script", authMiddleware_1.default, async (c) => {
    try {
        const body = await c.req.json();
        const { endpoints, iterations = 100, delay = 0.1 } = zod_1.z
            .object({
            delay: zod_1.z.number().min(0).max(10).default(0.1),
...
pythonTesting.get("/test-status", authMiddleware_1.default, async (c) => {
    try {
