...
const blockchainIntegration = new hono_1.Hono();
// Validation schemas
const syncTransactionsSchema = zod_1.z.object({
    chainId: zod_1.z.number().int().positive().default(42161), // Arbitrum
    tokenAddress: zod_1.z.string().optional(),
    walletAddresses: zod_1.z.array(zod_1.z.string().regex(/^0x[a-fA-F0-9]{40}$/))
});
const getTransactionHistorySchema = zod_1.z.object({
    fromBlock: zod_1.z.number().int().positive().optional(),
    limit: zod_1.z.number().int().positive().max(100).default(20),
    page: zod_1.z.number().int().positive().default(1),
    toBlock: zod_1.z.number().int().positive().optional(),
    tokenAddress: zod_1.z.string().optional(),
...
blockchainIntegration.post("/sync-transactions", authMiddleware_1.default, async (c) => {
    try {
        const body = await c.req.json();
        const { walletAddresses, tokenAddress, chainId } = syncTransactionsSchema.parse(body);
        const results = [];
        const errors = [];
...
blockchainIntegration.get("/transaction-history/:walletAddress", async (c) => {
    try {
        ...
        const { tokenAddress, fromBlock, toBlock, page, limit } = getTransactionHistorySchema.parse({ walletAddress, ...query });
    ...
    try {
        ...
        for (const tx of transactions) {
            ...
            balanceHistory.push({
                ...
                txHash: tx.txHash,
                type: tx.toAddress.toLowerCase() === walletAddress.toLowerCase()
                    ? "incoming"
                    : "outgoing"
            });
        }
...
blockchainIntegration.get("/contract-transactions", async (c) => {
    try {
        ...
        const skip = (page - 1) * limit;
        if (!contractAddress) {
            throw new ApiError_1.ApiError("Contract address is required", 400);
        }
        const [transactions, total] = await Promise.all([
            prisma.tokenTx.findMany({
                include: {
...
blockchainIntegration.post("/monitor-addresses", authMiddleware_1.default, async (c) => {
    try {
        const body = await c.req.json();
        const { addresses, tokenAddress, chainId } = zod_1.z
            .object({
            addresses: zod_1.z.array(zod_1.z.string().regex(/^0x[a-fA-F0-9]{40}$/)),
            chainId: zod_1.z.number().int().positive().default(42161),
            tokenAddress: zod_1.z.string().optional()
        })
            .parse(body);
...
blockchainIntegration.get("/monitored-addresses", authMiddleware_1.default, async (c) => {
    try {
...
blockchainIntegration.get("/transaction-stats", authMiddleware_1.default, async (c) => {
    try {
        const [totalTransactions, totalVolume, uniqueWallets, transactionsByToken, recentTransactions] = await Promise.all([
            prisma.tokenTx.count(),
...
blockchainIntegration.post("/sync-all", authMiddleware_1.default, async (c) => {
    try {
        ...
        const response = await fetch(`${c.req.url.replace("/sync-all", "/sync-transactions")}`, {
...
exports.default = blockchainIntegration;
