...
const getGamesSchema = zod_1.z.object({
    ...
    featured: zod_1.z.string().max(10).optional(),
    ...
    sortBy: zod_1.z.enum(["newest", "popular", "rating", "plays"]).optional(),
    source: zod_1.z.string().max(50).optional(),
    tag: zod_1.z.string().max(50).optional()
});
const getGames = async (c) => {
    try {
        ...
        const { category, search, source, sortBy, page, limit, tag, gameType } = parsedQuery;
        ...
        const transformedGames = games.map((game) => ({
            ...
            thumb1Url: toAbsolute(game.coverImageUrl || game.iconUrl || game.GameScreenshot?.[0]?.imageUrl),
            thumb2Url: toAbsolute(game.iconUrl || game.coverImageUrl || game.GameScreenshot?.[1]?.imageUrl),
            title: game.title,
