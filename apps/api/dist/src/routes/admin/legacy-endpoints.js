...
const deductCoinsSchema = zod_1.z.object({
    walletaddress: zod_1.z.string(),
    amount: zod_1.z.number().positive()
});
// Schema for withdrawal filters
const withdrawalFiltersSchema = zod_1.z.object({
    wallet: zod_1.z.string().optional(),
    min_usdt: zod_1.z.string().optional(),
    max_usdt: zod_1.z.string().optional(),
    min_tx: zod_1.z.string().optional(),
    max_tx: zod_1.z.string().optional(),
    include_all: zod_1.z.string().optional(),
    sort_by: zod_1.z.enum(['amount', 'tx']).optional().default('amount'),
    sort_dir: zod_1.z.enum(['desc', 'asc']).optional().default('desc'),
...
app.get('/list-user-withdrawals-csv', async (c) => {
    try {
        ...
        let filteredWithdrawals = Array.from(userWithdrawals.values());
        if (filters.wallet) {
            filteredWithdrawals = filteredWithdrawals.filter(w => w.walletaddress.toLowerCase().includes(filters.wallet.toLowerCase()));
        }
        if (filters.min_usdt) {
            const minUsdt = parseFloat(filters.min_usdt);
            filteredWithdrawals = filteredWithdrawals.filter(w => w.total_withdraw_usdt >= minUsdt);
        ...
        const csvRows = filteredWithdrawals.map(w => `${w.walletaddress},${w.total_withdraw_usdt},${w.total_withdraw_usd},${w.tx_count},${w.tx_unique}`).join('\n');
