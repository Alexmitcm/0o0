...
const exportRequestSchema = zod_1.z.object({
    type: zod_1.z.enum(['full', 'users', 'transactions']),
    includeArchives: zod_1.z.boolean().optional().default(true),
...
async function exportFullDatabase(includeArchives) {
    const data = {};
...
async function exportUsersData(includeArchives) {
    const data = {};
    data.users = await prisma.user.findMany({
        include: {
            userCoinBalance: true,
