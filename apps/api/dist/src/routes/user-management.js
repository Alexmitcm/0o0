...
const userManagement = new hono_1.Hono();
...
const premiumUpgradeSchema = zod_1.z.object({
    referer: zod_1.z
        .string()
        .regex(/^0x[a-fA-F0-9]{40}$/)
        .optional(),
...
function generateUserToken(walletAddress) {
    return (0, crypto_1.createHash)("sha256").update(walletAddress.slice(-15)).digest("hex");
}
// Helper function to check if user is banned
async function checkUserBan(walletAddress) {
...
userManagement.post("/signup", async (c) => {
    try {
        ...
        const { username, walletAddress, email, isEmailVerified, referer } = signupSchema.parse(body);
        ...
        if (usernameExists) {
            throw new ApiError_1.ApiError("Username already taken", 400);
        }
        // Generate user token
...
userManagement.post("/is-registered", async (c) => {
    try {
        const body = await c.req.json();
        const { walletAddress } = zod_1.z
            .object({ walletAddress: zod_1.z.string() })
            .parse(body);
        const user = await prisma.user.findUnique({
...
userManagement.post("/get-user-data", async (c) => {
    try {
        ...
        const banStatus = await checkUserBan(walletAddress);
        // Get tournament IDs
        const tournamentIds = user.tournamentParticipants.map((p) => p.tournament.id);
...
userManagement.post("/update-password", async (c) => {
    try {
        const body = await c.req.json();
        const { walletAddress, token, newPassword } = updatePasswordSchema.parse(body);
...
userManagement.get("/tree", async (c) => {
    try {
        ...
        const buildTree = (referrerAddress = null) => {
            return users
...
userManagement.get("/all-users", authMiddleware_1.default, async (c) => {
    try {
        const users = await prisma.user.findMany({
            include: {
                playHistory: {
